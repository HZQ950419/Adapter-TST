...
class Trainer:
    ...
    def __init__(
        ...
        self.model = model
        self.style_cls = style_classifier
    ...
    def _inner_training_loop(
        ...
        logger.info("  Adapter-TST version of adapter-transformers")
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        inputs["style_classifier"] = self.style_cls
        inputs["tokenizer"] = self.tokenizer
        inputs["tst_lambda"] = self.args.tst_lambda
        inputs["tst_lambda2"] = self.args.tst_lambda2
        inputs["num_label"] = [self.args.num_label_cls1, self.args.num_label_cls2]
        inputs["use_classification_head"] = self.args.use_classification_head
        ...
        if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
            ...
            loss = [loss_i / self.args.gradient_accumulation_steps for loss_i in loss]
        ...
        if self.do_grad_scaling:
            ...
            #update parameters of each 
            for i in range(len(self.args.adapter_names)):
                adapter_name = self.args.adapter_names[i]
                # ## TODO: This part is just for finetuning, delete this part for adapter traing
                # for param in model.parameters():
                #         param.requires_grad = False
                # print(model.shared_parameters)
                # if self.args.adapter_names[1-i] in model.shared_parameters:
                #     for param in model.shared_parameters[self.args.adapter_names[i]].values():
                #         print("this part has been exceeded")
                #         param.requires_grad = True
                #         exit()
                # for name, param in model.named_parameters():
                #     if self.args.adapter_names[i] in name:
                #         param.requires_grad_(True)
                #     else:
                #         param.requires_grad_(False)
                # model.train_adapter(adapter_name)
                if i == len(self.args.adapter_names)-1:
                    if loss[i] != 0.0:
                        loss[i].backward()
                else:
                    if loss[i] != 0.0:
                        loss[i].backward(retain_graph=True)
            #activate all the adapter before next iteration, otherwise, only one adapter will be activated.
            # model.set_active_adapters(ac.Parallel(self.args.adapter_names[0], self.args.adapter_names[1]))
            # loss.backward()

        return torch.mean(torch.tensor(loss)).detach()
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # for adapter text style tranfer, we have to replicate the input_ids and labels for metric calculation, however, the results won't be acurrate.
            # inputs_decode = self._prepare_input(inputs["input_ids"]) if args.include_inputs_for_metrics else None
            inputs_decode = self._prepare_input(inputs["input_ids"].repeat(self.args.num_label_cls1+self.args.num_label_cls2, 1)) if args.include_inputs_for_metrics else None
            labels = labels.repeat(self.args.num_label_cls1+self.args.num_label_cls2, 1)
        ...
        # for text style transfer we will also time the number of samples
        num_samples *= (self.args.num_label_cls1+self.args.num_label_cls2)

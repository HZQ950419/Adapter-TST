...
def cal_sc_loss(out, idx, cls, tokenizer, style, num_label):
    ...
    if num_label == 3:
        if style == 0:
            tgt_reward = tgt_cls[:, 0] - tgt_cls[:, 1] - tgt_cls[:, 2]
        elif style == 1:
            tgt_reward = tgt_cls[:, 1] - tgt_cls[:, 0] - tgt_cls[:, 2]
        elif style == 2:
            tgt_reward = tgt_cls[:, 2] - tgt_cls[:, 0] - tgt_cls[:, 1]

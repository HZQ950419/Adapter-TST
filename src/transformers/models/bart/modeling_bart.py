...
from ...utils.helper import cal_sc_loss
...
class BartForConditionalGeneration(BartModelWithHeadsAdaptersMixin, BartPretrainedModel):
    ...
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        style_labels: Optional[torch.LongTensor] = None,
        style_classifier: Optional[nn.Module] = None,
        ...
        tst_lambda: Optional[float] = None,
        tst_lambda2: Optional[float] = None,
        num_label: Optional[List] = None,
        use_classification_head: Optional[bool] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        # 1. extract style label idx and style transfer idx for each adapter
        batch_size = style_labels.size(0)
        style_idx = []
        st_idx = []
        for i in range(sum(num_label)):
            if i < num_label[0]:
                style_label = style_labels[:, 0]
                style_idx_ = torch.argwhere(style_label==i).squeeze().view(-1)
                style_idx.append(style_idx_)
                # extract style transfer idx
                full_idx = set(range(batch_size))
                st_idx.append(torch.tensor(list(full_idx-set(style_idx_.tolist()))).to(style_labels[0].device))
                
            else:
                style_label = style_labels[:, 1]
                style_idx_ = torch.argwhere(style_label==i-num_label[0]).squeeze().view(-1)
                style_idx.append(style_idx_)
                # extract style transfer idx
                full_idx = set(range(batch_size))
                st_idx.append(torch.tensor(list(full_idx-set(style_idx_.tolist()))).to(style_labels[i].device))
        
        # 2. split logits for each adapter
        adapter_logits = []
        for i in range(sum(num_label)):
            adapter_logits.append(lm_logits[batch_size*i: batch_size*(i+1)])

        # 3. extract reconstruction logits for each adapter
        rec_logits = []
        for i in range(sum(num_label)):
            if style_idx[i].size(0) != 0:
                rec_logits.append(torch.index_select(adapter_logits[i], 0, style_idx[i]))
            else:
                rec_logits.append(None)

        # 4. extract style transfer logits for each adapter
        st_logits = []
        for i in range(sum(num_label)):
            if style_idx[i].size(0) != batch_size:
                st_logits.append(torch.index_select(adapter_logits[i], 0, st_idx[i]))
            else:
                st_logits.append(None)
        
        # 5. calculate reconstruction loss and style transfer loss for each adapter
        masked_lm_loss = None
        if labels is not None:
            rec_loss = []
            st_loss = []
            ...
            for i in range(sum(num_label)):
                # reconstruction loss
                if rec_logits[i] != None:
                    rec_target =  torch.index_select(labels, 0, style_idx[i])
                    rec_loss.append(loss_fct(rec_logits[i].view(-1, self.config.vocab_size), rec_target.view(-1)))
                else:
                    rec_loss.append(0.0)
                #style transfer loss
                if st_logits[i] != None:
                    st_label = torch.index_select(labels, 0, st_idx[i])
                    idx = st_label.ne(-100).sum(-1)
                    if i < num_label[0]:
                        st_loss.append(cal_sc_loss(st_logits[i], idx, style_classifier[0], tokenizer, i, num_label[0]))
                    else:
                        st_loss.append(cal_sc_loss(st_logits[i], idx, style_classifier[1], tokenizer, i-num_label[0], num_label[1]))
                else:
                    st_loss.append(0.0)

            # 6. calculate loss for each adatper
            masked_lm_loss = [(1-tst_lambda)*rec_loss[i] + tst_lambda*st_loss[i] for i in range(num_label[0])]
            if num_label[1] != 0:
                 masked_lm_loss += [(1-tst_lambda2)*rec_loss[i] + tst_lambda2*st_loss[i] for i in range(num_label[0], sum(num_label))]
            # masked_lm_loss = [st_loss[i] for i in range(sum(num_label))]
